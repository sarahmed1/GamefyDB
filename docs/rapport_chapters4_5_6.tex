...
real decision-making? The answer, delivered through five iterative sprints, is the
GamefyDB system.
...
Sprint~5 closed the loop with a conversational assistant. Built on top of Streamlit
and the Gemini API, it lets the owner ask questions in English, French, or Arabic ---
by typing or by voice --- and returns both a written answer and, when relevant, a
Plotly chart. The assistant reads the exact same CSV files Power~BI consumes, so
there is one source of truth, and the startup greeting changes when the anomaly
detector has flagged something in the previous seven days, so the most pressing
piece of information surfaces without anyone having to look for it.
