...
This chapter sets up the context for the rest of the report. It starts with a
presentation of Gamefy Academy, covering who they are, how they operate, and what data
they generate. From there it identifies the problems with the existing system,
states the question the project tries to answer, and describes the solution that
was built. The chapter ends with a discussion of the project management methodology.

\section{Host Organisation Presentation}

\textbf{Gamefy Academy} is a gaming center located in \textbf{La Soukra, Tunis}.
It opened in 2025 and targets a young clientele looking for a high-quality gaming
experience in a social setting. The center offers PC gaming on high-end hardware,
PlayStation~5 consoles, a private VIP gaming booth, and a snack bar. It positions
itself in a market that has grown significantly in Tunisia over the last few years,
as competitive gaming and LAN events have become increasingly popular.

% *TODO* insert Gamefy logo image if available
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.3\textwidth]{gamefy_logo.png}
%   \caption{Gamefy Academy logo.}
% \end{figure}

\subsection{Organisational Structure}

Operations are managed by a small team. Five cashier accounts handle all
customer-facing activity: \textit{taktek}, \textit{yassine}, \textit{youssef},
\textit{monta}, and \textit{guds}. There is also an \textit{admin} account
used for system-level operations. Each cashier covers one or more terminals during
a shift and is responsible for opening sessions, recording cash transactions, and
managing stock sales.

All activity is automatically logged by the center's point-of-sale software, which
generates four Excel reports that can be exported at any time: one for cash
transactions, one for session data, one for stock movements, and one for member
statistics.

\subsection{Services Provided}

The center has 16 active terminals distributed across three categories:

\begin{itemize}
  \item \textbf{Standard PC workstations} (GAMEFY01 to GAMEFY10): ten high-spec
        gaming computers. Sessions are billed by duration and can be of five types:
        Standard, Member, Free, Time Limited, or Administrator.

  \item \textbf{PlayStation~5 consoles} (PS5~(1) and PS5~(2)): two console stations
        running the same session type system as the PCs.

  \item \textbf{VIP gaming booth} (GAMEFY-VIP): a private room with a premium setup
        at a higher hourly rate, available by reservation.

  \item \textbf{Administrative workstations}: three DESKTOP machines used by staff
        that appear in session data under the Administrator type.
\end{itemize}

Alongside gaming, the center runs a snack bar stocked with drinks, cookies, and
snack items. These sales are tracked through the stock management system. A loyalty
membership programme offers registered members reduced rates and tracked usage.

\section{Project Presentation}

\subsection{General Context}

This project was completed as part of the final-year requirements for a Bachelor's
degree in Business Intelligence at the Higher Institute of Computer Science of
Mahdia, Faculty of Economics and Management. The internship ran over four months at
Gamefy Academy and was jointly supervised by an academic supervisor from ISIMA and
the center's owner.

The project sits at the intersection of data engineering and business analytics.
The starting point is four raw Excel files. The end goal is a repeatable,
automated system that produces clean data, interactive dashboards, and machine-learning
forecasts, none of which existed at the start of the internship.

\subsection{Analysis of the Existing System}

When I arrived at Gamefy, the data workflow was straightforward: the point-of-sale
...
visualised them. If the owner wanted to know something about the business (which cashier had the
best month, which terminal was most popular, whether revenue was trending up),
the only option was to open the correct Excel file and build a pivot table manually.

In practice, this meant it almost never happened. The friction of going from
...
Going through the files in detail revealed a second layer of problems beyond
just the lack of tooling. The data quality issues in the source files are not
visible to someone reading them in Excel, but they make automated processing
significantly harder than it should be.

\begin{table}[H]
  \centering
  \caption{Summary of data quality issues found in the source files.}
  \begin{tabular}{p{3.5cm} p{9.5cm}}
    ...
    Page-break artefacts
      & The export software inserts a \textit{Page~X/Y} row, two blank rows, and
        a full header repetition approximately every 40 data rows. \\[4pt]
    Non-standard number format
      & TND amounts use a narrow no-break space (\texttt{\textbackslash u202f})
        as the thousands separator and a comma as the decimal separator. Standard
        parsers reject both. \\[4pt]
    Merged-cell column shift
      & Merged cells in the Excel layout shift the column index where data actually
        lands relative to the visible header, making automatic header detection
        unreliable. \\[4pt]
    Missing cashier names
      & Sub-total rows inserted between data rows carry a generic label in the
        cashier column instead of the actual cashier account name. \\[4pt]
    Inconsistent totals
      & Some session and stock rows have recorded totals that do not match the
        sum of their component columns, indicating data-entry or rounding errors
        in the source software. \\[4pt]
    Footer rows
      & The members file ends with a \textit{Report Result\,:} footer row that
        contains numeric totals in what appears to be a data position. \\
    \bottomrule
  \end{tabular}
\end{table}

None of these issues were documented. Each one was discovered by inspecting actual
rows from the files and working out why the parsing was failing.

\subsection{Problem Statement}

The center collects detailed operational data continuously but derives almost no
value from it. The question this project addresses is: \textit{how can these raw,
artefact-laden Excel exports be transformed reliably and repeatably into a clean
dataset that supports real decision-making?}

The word ``reliably'' matters. A one-time manual clean is not a solution. What the
center needs is a process that can be rerun every time the files are updated, always
producing correct output.

\subsection{Proposed Solution}

The solution developed is called \textbf{GamefyDB}. It is a Python-based system
with three components:

\begin{enumerate}
  \item \textbf{\texttt{gamefydb/pipeline.py}} handles the full ETL chain: reading
        the four source files, stripping all formatting artefacts, converting every
        column to the correct type, correcting data quality issues, and building the
        six-table star schema. A single function (\texttt{run\_pipeline()}) runs the
        entire chain and returns the output.

  \item \textbf{\texttt{gamefydb/forecaster.py}} contains five independent
        forecasting functions that read the cleaned data and produce forward-looking
        CSV files for demand planning.

  \item \textbf{\texttt{run.py}} is the command-line entry point. It calls the
        pipeline, writes the star-schema tables to \texttt{output/powerbi\_star/},
        then calls each forecasting function and writes results to
        \texttt{output/forecasts/}.
\end{enumerate}

The output feeds into a Power~BI report (\texttt{gamefyyy.pbix}) with four
dashboard pages covering revenue, rankings, live operations, and shift reporting.

\section{Project Management Methodology}

\subsection{The GIMSI Method}

GIMSI is a project management methodology designed specifically for Business
Intelligence projects. It provides a structured approach to the full lifecycle
of a decision-support system, from identifying needs to operational deployment.

% *TODO* insert GIMSI phases diagram here
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.75\textwidth]{gimsi_phases.png}
%   \caption{Main phases of the GIMSI methodology.}
%   \label{fig:gimsi}
% \end{figure}

\subsubsection{GIMSI Objectives}

GIMSI is designed to keep all project phases consistent with each other and with the
organisation's real needs. Its four main objectives are: organising the project
process methodically, guaranteeing consistency across all deliverables, bringing
all stakeholders into the process collaboratively, and preserving knowledge produced
during the project for future reuse.

\subsubsection{Main Phases of GIMSI}

The methodology breaks down into five phases. The Preliminary Study justifies the
project and analyses its context. The Opportunity Study evaluates feasibility and
candidate solutions. The Detailed Study produces functional requirements and
conceptual models. The Implementation phase covers development and testing. Finally,
Permanent Improvement handles post-deployment refinement based on feedback.

\subsubsection{Advantages and Limits}

GIMSI is well-suited to projects where the business requirements are stable and
can be fully specified before implementation begins. It is strong on documentation
and traceability. However, it is less flexible when the problem evolves during
implementation, which is exactly the situation in a data project where source data
quality is unknown until you inspect the actual files.

\subsection{Scrum Methodology}

Scrum is an agile framework built around short delivery cycles called sprints,
typically two to four weeks long. Each sprint ends with a working product increment,
a review with stakeholders, and a retrospective to improve the next cycle.
...
Three roles define the Scrum team. The \textit{Product Owner} owns the backlog and
sets priorities based on business value. The \textit{Scrum Master} facilitates the
process and removes anything blocking the team. The \textit{Development Team} is
responsible for delivering each sprint's increment.

The ceremonies (Sprint Planning, Daily Scrum, Sprint Review, and Sprint
Retrospective) create a rhythm that keeps the team aligned and gives stakeholders
regular visibility into progress without requiring a formal sign-off at every step.

\subsubsection{Advantages}

The strongest advantage of Scrum for this type of project is the ability to respond
to change. When you discover during Sprint~1 that the source data has six different
quality problems you did not know about before, Scrum lets you handle them without
derailing the plan. The sprint boundary is a natural point to reassess and adjust.

\subsection{Adopted Methodology}

I chose Scrum. The deciding factor was uncertainty: when I started the internship,
I had no way of knowing what the data quality issues were going to be. Designing
everything upfront would have meant making assumptions that turned out to be wrong.
Scrum let me inspect the real files in Sprint~1, fix what I found, and move on to
the schema in Sprint~2 only once the cleaning was solid.

Sprint~0 was used as a planning phase where no code was written. Four implementation
sprints followed.

\section{Conclusion}

Gamefy Academy had the data but not the infrastructure to use it. This chapter
identified the gap (messy files, no automation, no dashboard) and introduced
the system built to fill it. Chapter~2 covers the preparation work done before
...
Sprint~0 is the planning sprint. Nothing is deployed, but everything that comes
after depends on getting this right. During this phase I identified the actors,
documented the requirements, set up the product backlog, chose the architecture,
and designed the dimensional model. In practice it took longer than the two weeks
planned, because understanding the source files well enough to write reliable
requirements meant inspecting the real data in detail.

\section{Requirements Specification}

\subsection{Identification of System Actors}

Two actors interact with GamefyDB.

The \textbf{Data Analyst} is the person who runs the pipeline, validates the output,
and maintains the dashboards. In this project that was me during the internship, but
the system is designed to be runnable by anyone with Python installed. The analyst
is the only actor who directly triggers the system.

The \textbf{Downstream System} (Power~BI or any other BI tool) is the consumer of
the output. It does not trigger anything, but its requirements determine the output
format: UTF-8-BOM encoding, clean column names, integer surrogate keys, and
referential integrity across tables.

\subsection{Functional Requirements}

The functional requirements were written after inspecting all four source files.
They are ordered by the pipeline stage they belong to.

\begin{enumerate}
  \item Read all four \texttt{.xls} files reliably without encoding errors.
  \item Remove page-break artefacts (repeated headers, page markers, blank rows)
        from every file before any processing.
  \item Parse TND amounts that use a narrow no-break space as the thousands
        separator and a comma as the decimal separator.
  \item Parse all datetime columns in \texttt{DD.MM.YYYY~HH:MM:SS} format into
        typed datetime objects.
  \item Convert duration strings in \texttt{Xh~Y~min} format into numeric minutes.
  \item Correct missing or invalid cashier names by propagating known cashier
        account names through sub-total rows.
  \item Impute missing amounts, quantities, and totals using the relationships
        between columns (e.g.\ total = quantity $\times$ unit price).
  \item Build four dimension tables with surrogate integer primary keys.
  \item Build two fact tables with foreign-key references to the dimensions.
  \item Export all six tables as UTF-8-BOM encoded CSV files.
  \item Produce five forecast outputs for revenue, member activity, peak hours,
        session volume, and stock replenishment.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Reproducibility}: given the same four input files, the pipeline
        must always produce byte-identical output.
  \item \textbf{Portability}: no hardcoded paths; the pipeline must run on any
        machine with the required Python packages, on both Windows and Linux.
  \item \textbf{Maintainability}: column index mappings are defined explicitly in
        each loader function so that a format change in the source files can be
        corrected in one place.
  \item \textbf{Testability}: the parsing and cleaning functions are independently
        testable and covered by a \texttt{pytest} suite.
\end{enumerate}
...
The Analyst triggers two primary use cases: \textit{Run ETL Pipeline} and
\textit{Generate Forecasts}. The pipeline includes three subordinate steps (Load Source Files, Clean and
Correct Data, Build Star Schema), modelled as
\texttt{<<include>>} relationships. Forecast generation extends the base run
since it is invoked from the same command. The Downstream System receives the
output through a Load into Power~BI use case.

Table~\ref{tab:uc_pipeline} gives the text description of the main use case.

\begin{table}[H]
\centering
\caption{Text description of use case ``Run ETL Pipeline''.}
\label{tab:uc_pipeline}
\begin{tabular}{L R}
  \toprule
  \textbf{Field} & \textbf{Description} \\
  \midrule
  Use Case Name  & Run ETL Pipeline \\
  Actor          & Data Analyst \\
  Preconditions  & The four \texttt{.xls} source files are present in the
                   \texttt{excel/} directory. Python and all required packages
                   are installed. \\
  Main Flow      & 1. The analyst runs \texttt{python run.py}. \newline
                   2. The system locates the four source files. \newline
                   3. Each file is loaded, artefacts are removed, and columns
                      are typed and corrected. \newline
                   4. The star schema is built from the cleaned DataFrames. \newline
                   5. Six CSV files are written to \texttt{output/powerbi\_star/}. \newline
                   6. Five forecast CSVs are written to \texttt{output/forecasts/}. \newline
                   7. A completion message is printed to the console. \\
  Alternative Flow & If a source file is missing, the system raises a
                   \texttt{FileNotFoundError} listing the missing files and
                   halts before any output is written. \\
  Postconditions & All eleven output files exist in the output directories,
                   are correctly encoded, and contain no artefact rows. \\
  \bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Product Owner}: Mr.\ Aymen Jadallah, owner of Gamefy Academy.
        He defined business requirements, reviewed sprint outputs, and validated
        that dashboard KPIs matched his operational needs.

  \item \textbf{Scrum Master}: Mr.\ Sofien Boutaib, PhD in Computer Science from
        ISIMA and academic supervisor. He ensured Scrum ceremonies were followed
        and provided technical and methodological guidance.

  \item \textbf{Development Team}: Sarra Mediouni, the sole developer on this
        project, responsible for all implementation including the pipeline, the
        forecasting module, and the Power~BI dashboards.
\end{itemize}

\subsection{Product Backlog}

The product backlog defines every user story that the project needs to deliver.
Sprint~4 was not in the original backlog; it was added after Sprint~3 was validated,
in response to the Product Owner asking for demand-planning support.

\begin{longtable}{>{\bfseries}p{1.4cm} p{4.3cm} p{5.3cm} c}
  ...
  US-01 & As an analyst, I want to read all four source files without errors.
        & Implement one loader per file; map column indices empirically.
        & 1 \\[3pt]
  ...
  US-05 & As an analyst, I want four dimension tables with surrogate keys.
        & Build dims inside \texttt{build\_star\_schema()}.
        & 2 \\[3pt]
  US-06 & As an analyst, I want two fact tables with FK references.
        & Build facts; resolve all FKs; use nullable Int64 for optional refs.
        & 2 \\[3pt]
  US-07 & As an analyst, I want the schema exported as UTF-8-BOM CSV files.
        & Write tables to \texttt{output/powerbi\_star/} from \texttt{run.py}.
        & 2 \\[3pt]
  US-08 & As an analyst, I want KPIs visualised in interactive dashboards.
        & Build \texttt{gamefyyy.pbix}: date dim, relationships, DAX, 4 pages.
        & 3 \\[3pt]
  US-09 & As an owner, I want a revenue forecast for the next 4 weeks and 3 months.
        & Implement \texttt{forecast\_revenue()} using Prophet.
        & 4 \\[3pt]
  ...
  US-11 & As an owner, I want to see the busiest hours and days.
        & Implement \texttt{forecast\_peak\_hours()}: two separate CSV files.
        & 4 \\[3pt]
  US-12 & As an owner, I want a forecast of total activity volume.
        & Implement \texttt{forecast\_session\_volume()} using Prophet.
        & 4 \\[3pt]
...
The project ran over four implementation sprints after Sprint~0:

\begin{itemize}
  \item \textbf{Sprint 1: Loading and Cleaning}: all four loader functions,
        the artefact-filtering logic, the type-conversion helpers, and the cashier
        correction. Validated against the real source files.
  \item \textbf{Sprint 2: Star Schema and Export}: the schema builder and the
        CSV export step, with referential integrity checks across all six tables.
  ...
  \item \textbf{Sprint 4: Machine Learning Forecasting}: the five forecasting
        functions, member segmentation, and anomaly detection, added after
        Sprint~3 was validated at the Product Owner's request.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Sprint 4: ML Forecasting        & 17/04/2026 & 30/04/2026 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Figure~\ref{fig:architecture} shows the overall architecture of the GamefyDB system.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{pipeline.png}
  \caption{Technical architecture of the GamefyDB pipeline.}
  \label{fig:architecture}
\end{figure}

The design is intentionally flat. There are three layers. The source layer is just
the four Excel files in the \texttt{excel/} directory. The processing layer is two
Python modules: \texttt{pipeline.py} handles loading, cleaning, and schema
construction, while \texttt{forecaster.py} produces the predictive outputs. The
output layer is two folders that Power~BI reads from directly.

Keeping everything in two modules rather than a package of six or eight files was
a deliberate choice. The cleaning logic for each file is tightly coupled to how
that file is structured; separating it into a generic cleaner would have added
abstraction without adding clarity.

\section{Overall Design of the Data Warehouse}

\subsection{Data Collection}

The source data is four \texttt{.xls} files exported by the point-of-sale software,
covering Gamefy's activity from June 2025 through April 2026. Table~\ref{tab:sourcefiles}
summarises the four files.
  ...
  \begin{tabular}{p{3.5cm} p{9cm}}
    ...
    14-06-2025 cash.xls
      & One row per cash transaction. Columns include cashier, datetime,
        income/expense type, payment method, category, and TND amount.
        Data trimmed to June 2025--April 2026. \\[4pt]
    14-06-2025 session.xls
      & One row per gaming session. Columns include cashier, terminal, session type,
        duration, and revenue broken down into usage, orders, USB, and discount. \\[4pt]
    14-06-2025 stock.xls
      & One row per stock movement. Columns include cashier, datetime, item name,
        category, direction (in/out), quantity, unit price, and total. \\[4pt]
    memeber DATA 01-09-2025.xls
      & One row per loyalty member. Contains cumulative stats: total session
        duration, total spending broken down by type. \\
    \bottomrule
  \end{tabular}
\end{table}
...
The KPIs were defined during Sprint~0 in discussion with the Product Owner. They
fall into four groups as shown in Table~\ref{tab:kpis}.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3cm} p{10cm}}
    ...
    Sessions
      & Total session count, average session duration (minutes), free vs.\
        paid session split, total discount given, session revenue by terminal,
        session revenue by terminal type. \\[4pt]
    Stock
      & Total units sold per item, total snack bar revenue, average unit
        price per item, current stock level (net in minus out). \\[4pt]
    Members
      & Total active members, average spend per member, total session
        duration across all members, orders transferred, USB data usage. \\
    \bottomrule
  \end{tabular}
\end{table}
...
Bill Inmon's approach, often called top-down, starts by designing a single fully
normalised enterprise data warehouse that covers the entire organisation, then
derives subject-oriented data marts from it. The normalisation ensures minimal
redundancy and a single source of truth, but it requires the entire warehouse to
be designed before any reports can be produced.

% *TODO* insert Inmon architecture diagram
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.7\textwidth]{inmon_arch.png}
%   \caption{Inmon data warehouse architecture.}
%   \label{fig:inmon}
% \end{figure}

\subsubsection{Kimball Approach}

Ralph Kimball's approach, bottom-up, models each business process as an independent
star schema and integrates them later through shared conformed dimensions. It gets
usable output into analysts' hands faster, at the cost of some potential redundancy
between data marts.

% *TODO* insert Kimball architecture diagram
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.7\textwidth]{kimball_arch.png}
%   \caption{Kimball data warehouse architecture.}
%   \label{fig:kimball}
% \end{figure}

\subsubsection{Adopted Approach}

I went with Kimball. With only four business domains, one physical location, and a
tight project timeline, designing a full normalised EDW first would have taken the
entire internship without producing any dashboards. The bottom-up approach let me
validate each output table against real data during Sprint~2 and adjust before
moving on. The shared \textit{dim\_cashier} and \textit{dim\_terminal} dimensions
provide the cross-domain integration that Kimball requires without adding unnecessary
complexity.
...
\begin{itemize}
  \item \textbf{Star Schema}: fact tables reference dimension tables directly with
        no normalisation of the dimensions. Simple, fast, and easy to understand.
        This is what Power~BI is optimised for.

  \item \textbf{Snowflake Schema}: dimension tables are further normalised into
        sub-dimensions. Reduces some storage redundancy but adds joins at query time.
        Power~BI handles joins, but each additional one adds some complexity to the
        model.

  \item \textbf{Galaxy Schema}: multiple fact tables share conformed dimensions.
        This is essentially what GamefyDB needs, since both fact tables share
        the cashier and terminal dimensions.
\end{itemize}

\subsection{Choice of Conceptual Model}

The chosen model is a \textbf{star schema} with two fact tables sharing four
conformed dimensions, which in practice is close to a galaxy schema. The dimensions
are not normalised further (no snowflaking) because the dimensional tables are small
and the simplicity of the star structure is a genuine advantage in Power~BI.

Figure~\ref{fig:starschema} shows the final logical model.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{star.png}
  \caption{Logical model of the GamefyDB data warehouse.}
  \label{fig:starschema}
\end{figure}

Table~\ref{tab:star_schema} lists the six output tables.
...
Dimension & \texttt{dim\_terminal} & terminal\_id (PK), terminal\_name, terminal\_type \\
...
\midrule
Fact & \texttt{fact\_transaction} & cashier\_id (FK), terminal\_id (FK), item\_id (FK), \\
     &                            & member\_id (FK), date, amount, payment, category, type \\
Fact & \texttt{fact\_session}     & cashier\_id (FK), terminal\_id (FK), member\_id (FK), \\
     &                            & session\_type, duration\_min, order\_transfer, \\
     &                            & usage, discount, total \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Python 3.x}: the main implementation language. Chosen for its
        mature data engineering ecosystem, native Excel reading support via pandas,
        and access to Prophet for time-series forecasting.

  \item \textbf{pandas}: used throughout for reading Excel files with
        \texttt{read\_excel}, all DataFrame operations, and writing CSV output.

  \item \textbf{NumPy}: used for vectorised nearest-neighbour matching when
        resolving item foreign keys from transaction amounts.

  \item \textbf{Prophet}: Meta's open-source time-series library, used in three
        of the five forecasting functions. Handles irregular time series and weekly
        seasonality without manual parameter tuning.

  \item \textbf{Power~BI Desktop}: used to load the output CSV files, define the
        data model relationships, write DAX measures, and build the four dashboard
        pages.

  \item \textbf{pytest}: automated testing for the parsing and cleaning helpers.

  \item \textbf{VS~Code}: code editor with the Python extension.

  \item \textbf{Git / GitHub}: version control for all code, configuration, and
        documentation.

  \item \textbf{Overleaf}: online \LaTeX{} editor used to write and compile this
        report.
\end{itemize}

\section{Conclusion}

By the end of Sprint~0, the scope was defined, the backlog was in place, and the
architectural decisions had been made. The choice of Kimball over Inmon, star schema
over snowflake, and one unified pipeline module over separate stage files all came
from the same place: the project is small, the data quality is uncertain, and the
timeline is short. Chapter~3 covers the first implementation sprint.
...
This page answers the first question any owner asks: how much money did we make?

The top row has four KPI cards: Total Revenue, Total Expenses, Net Income, and the
month-over-month revenue change. Below them, a line chart plots revenue over time,
making it easy to see weekly patterns and monthly trends at a glance. A bar chart
below that breaks revenue down by hour of day, using the Transaction Hour calculated
column on the axis. A donut chart shows the split between cash and credit card
payments. Month name and year slicers at the top of the page filter all of these
visuals simultaneously.

\begin{figure}[H]
  \centering
  \fbox{\begin{minipage}{0.95\textwidth}\centering\vspace{3cm}
    \textit{[Screenshot: Revenue Overview dashboard page --- add \texttt{dashboard\_revenue.png}]}
  \vspace{3cm}\end{minipage}}
  \caption{Revenue Overview dashboard page.\label{fig:dash_revenue}}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Analysis of the Revenue Overview dashboard visuals.\label{tab:rev_analysis}}
  \begin{tabular}{p{3.5cm} p{2.5cm} p{7cm}}
    \toprule
    \textbf{Visual} & \textbf{Type} & \textbf{Purpose} \\
    \midrule
    KPI cards (Revenue, Expenses, Net Income)
      & Card
      & Immediate snapshot of financial health for the selected period. \\[4pt]
    Revenue MoM\%
      & Card with trend
      & Shows whether revenue is growing or declining month-over-month. \\[4pt]
    Revenue over time
      & Line chart
      & Reveals seasonal patterns and anomalies across the dataset period. \\[4pt]
    Revenue by hour of day
      & Bar chart
      & Identifies peak revenue hours to guide opening hours and staffing. \\[4pt]
    Revenue by payment method
      & Donut chart
      & Shows the cash vs.\ card split; useful for cash management planning. \\
    \bottomrule
  \end{tabular}
\end{table}
...
This page answers: who and what is contributing the most?

A horizontal bar chart ranks all five cashiers by Total Revenue, sorted descending.
This gives an immediate picture of which shifts are most productive. A second bar
chart ranks stock items by Item Sales Revenue. Below that, a clustered bar chart
compares session revenue across the three terminal types. A detail table lists every
individual terminal with its session count and revenue, so underperforming machines
can be spotted.

\begin{figure}[H]
  \centering
  \fbox{\begin{minipage}{0.95\textwidth}\centering\vspace{3cm}
    \textit{[Screenshot: Rankings dashboard page --- add \texttt{dashboard\_rankings.png}]}
  \vspace{3cm}\end{minipage}}
  \caption{Rankings dashboard page.\label{fig:dash_rankings}}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Analysis of the Rankings dashboard visuals.\label{tab:rank_analysis}}
  \begin{tabular}{p{3.5cm} p{2.5cm} p{7cm}}
    \toprule
    \textbf{Visual} & \textbf{Type} & \textbf{Purpose} \\
    \midrule
    Cashier bar chart
      & Horizontal bar
      & Ranks cashiers by revenue generated; supports performance review. \\[4pt]
    Item sales bar chart
      & Horizontal bar
      & Identifies best-selling and slow-moving snack bar items. \\[4pt]
    Terminal type comparison
      & Clustered bar
      & Compares revenue across PC, PlayStation, and VIP station types. \\[4pt]
    Terminal detail table
      & Table
      & Granular view per terminal for maintenance and investment decisions. \\[4pt]
    Session KPI cards
      & Cards
      & Avg session duration, total discount, and free-time session rate. \\
    \bottomrule
  \end{tabular}
\end{table}
...
This page is designed for a shift check-in. Everything on it is filtered to
today's date without any slicer interaction needed.

The top row shows Revenue Today, Expenses Today, Net Income Today, and Transactions
Today. Below those are cards for stock in and stock out today, giving a quick sense
of how the snack bar is performing. The main visual is a stock table showing the
Current Stock measure per item, with conditional formatting highlighting anything
below a safe level in red. A bar chart at the bottom shows session counts by terminal
type, giving a sense of how busy each area is.

\begin{figure}[H]
  \centering
  \fbox{\begin{minipage}{0.95\textwidth}\centering\vspace{3cm}
    \textit{[Screenshot: Live Ops dashboard page --- add \texttt{dashboard\_liveops.png}]}
  \vspace{3cm}\end{minipage}}
  \caption{Live Ops dashboard page.\label{fig:dash_liveops}}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Analysis of the Live Ops dashboard visuals.\label{tab:ops_analysis}}
  \begin{tabular}{p{3.5cm} p{2.5cm} p{7cm}}
    \toprule
    \textbf{Visual} & \textbf{Type} & \textbf{Purpose} \\
    \midrule
    Today's financial KPIs
      & Cards
      & Real-time snapshot of today's revenue and net income. \\[4pt]
    Stock in / stock out today
      & Cards
      & Immediate view of how much stock has moved today. \\[4pt]
    Current stock table
      & Table with CF
      & Shows net inventory per item; conditional formatting flags low stock. \\[4pt]
    Sessions by terminal type
      & Bar chart
      & Shows today's terminal load without needing to check session logs. \\
    \bottomrule
  \end{tabular}
\end{table}
...
This page is the end-of-shift summary. Two slicers (a single-select cashier dropdown and a date-range picker) filter
every visual on the page. With those set,
the owner gets a complete picture of what a specific cashier did on a specific day.

Financial cards show Total Revenue, Session Revenue, Item Sales Revenue, and Shift
Net for the selected cashier and date. Activity cards show Transaction Count, Session
Count, Item Units Sold, and Discount Total. Two bar charts at the bottom show items
sold and session revenue by terminal type, giving the owner context for the numbers
on the cards.

\begin{figure}[H]
  \centering
  \fbox{\begin{minipage}{0.95\textwidth}\centering\vspace{3cm}
    \textit{[Screenshot: Shift Report dashboard page --- add \texttt{dashboard\_shift.png}]}
  \vspace{3cm}\end{minipage}}
  \caption{Shift Report dashboard page.\label{fig:dash_shift}}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Analysis of the Shift Report dashboard visuals.\label{tab:shift_analysis}}
  \begin{tabular}{p{3.5cm} p{2.5cm} p{7cm}}
    \toprule
    \textbf{Visual} & \textbf{Type} & \textbf{Purpose} \\
    \midrule
    Cashier slicer
      & Dropdown slicer
      & Selects a specific cashier to filter the entire page. \\[4pt]
    Date slicer
      & Date range slicer
      & Restricts to a specific shift date or range. \\[4pt]
    Financial KPI cards
      & Cards
      & Shows exactly what the cashier collected and spent in the selected shift. \\[4pt]
    Activity KPI cards
      & Cards
      & Shows transaction volume, session count, and discounts given. \\[4pt]
    Items sold bar chart
      & Bar chart
      & Shows which items were sold most during the selected shift. \\[4pt]
    Session revenue by type
      & Bar chart
      & Shows which terminal category drove the most session revenue. \\
    \bottomrule
  \end{tabular}
\end{table}
...
I reviewed all four pages with the Product Owner during the sprint review. Revenue
figures on Page~1 were cross-checked against the raw cash file for a full month and
matched exactly. Cashier rankings on Page~2 were verified manually. The today filters
on Page~3 were tested by changing the system date. The Shift Report was tested for
three different cashiers against calculations done independently in a spreadsheet.
All values agreed.

The Product Owner validated the sprint and then asked whether the same data could
be used to forecast future demand. That request became Sprint~4.

\section{Conclusion}

Sprint~3 delivered a validated, four-page Power~BI report covering all the KPIs
defined in Sprint~0. The data went from a stack of Excel files to interactive,
filterable dashboards that can be refreshed with a single command. Chapter~6 covers
the forecasting extension built in Sprint~4.
...
stationary. Figure~\ref{fig:stationarity_rolling} shows the rolling mean plots
for all three series.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{stationarity_rolling.png}
  \caption{Rolling mean and standard deviation for the three forecast series.
           A drifting mean or widening band indicates
           non-stationarity.\label{fig:stationarity_rolling}}
\end{figure}

The numerical results from the ADF test are shown in Table~\ref{tab:adf_results}
(values are generated by the pipeline and saved to
\texttt{output/forecasts/stationarity\_tests.csv}).

\begin{table}[H]
  \centering
  \caption{ADF stationarity test results.\label{tab:adf_results}}
  \begin{tabular}{lrrrl}
    \toprule
    \textbf{Series} & \textbf{ADF stat} & \textbf{p-value} &
    \textbf{Critical (5\%)} & \textbf{Verdict} \\
    \midrule
    Revenue (TND, daily)     & $-3.927$ & $0.002$ & $-2.871$ & Stationary \\
    Session volume (daily)   & $-4.001$ & $0.001$ & $-2.871$ & Stationary \\
    Member activity (weekly) & $-6.592$ & $< 0.001$ & $-2.928$ & Stationary \\
    \bottomrule
  \end{tabular}
\end{table}

In any case, neither Prophet nor SARIMA required a manual pre-transformation here.
Prophet handles trends internally through its piecewise linear component, and
\texttt{auto\_arima} figures out the right amount of differencing on its own via
the $d$ parameter. So even if the series turned out to be non-stationary, the
models would still work correctly without me having to apply a log transform
beforehand.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{model_comparison_bar.png}
...
A conversational chatbot (\texttt{app.py}), built on the Gemini API, was added
at the end of the sprint so that the owner can ask plain-language questions and
get answers with auto-generated charts, without needing Power~BI open.
...
interpretable required some iteration on the feature selection. A conversational
chatbot backed by the Gemini API was added at the end to make the outputs accessible
without needing Power~BI.
