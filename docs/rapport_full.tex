...
The \texttt{\_fill\_cashier()} function exploits this property. It sets any value
in the cashier column that is not in the known-cashiers list to \texttt{NaN},
then applies a forward fill followed by a backward fill. The forward fill
propagates the most recent real cashier name down through subsequent sub-total
rows; the backward fill handles any leading sub-total rows that appear before
the first real cashier entry.
...
This page is the end-of-shift summary. Two slicers, a single-select cashier
dropdown and a date-range picker, filter every visual on the page. With those
set, the owner gets a complete picture of what a specific cashier did on a
specific day.
...
Prophet is the forecasting library I ended up relying on for three of the five
forecasts in this project. It comes from Meta's research team, was first published
in 2017, and the underlying idea is to fit a small set of interpretable components
(trend, seasonality, holiday effects) rather than learn arbitrary patterns from
scratch. That tradeoff matters here. The Gamefy dataset is small by machine-learning
standards, around seven months at the time of model selection, and has the kind of
irregularities that come from a real business: days when the center was closed,
weeks with unusual peaks during exam breaks, and a few one-off promotions. Prophet
absorbs those without preprocessing, which was the practical reason I picked it
over the alternatives in the next subsection.
...
Prophet was chosen primarily for two reasons. First, the dataset at the time of
the model-selection decision covered roughly seven months
(Section~\ref{sec:april_validation} describes the later extension to ten months),
which is far below the threshold where deep learning models become reliable.
Second, Prophet's handling of irregular series (days with no transactions, weeks
with unusual patterns) is built in, whereas ARIMA would require explicit
preprocessing.

\subsection{How Prophet Works}

Under the hood Prophet adds three things together to produce a prediction at any
point in time:
...
For the Gamefy series, $T(t)$ is the slow drift in revenue or session counts
across months: a piecewise linear curve that can change slope at automatically
detected breakpoints. $S(t)$ captures the weekly pattern that dominates this
dataset, weekends and Fridays being noticeably busier than weekdays, modelled as
a Fourier series so that the seven-day shape is smooth rather than a step
function. $\epsilon_t$ is whatever the first two components fail to explain.
Because the components stay separate inside the fitted model, I could inspect
the weekly seasonality directly to sanity-check that Prophet had learned the
right pattern before trusting any of the forward predictions.

In code the integration is short. Each forecasting function reshapes its series
into a two-column DataFrame (\texttt{ds} for the date, \texttt{y} for the value),
calls \texttt{Prophet().fit(df)}, generates a future-date frame with
\texttt{make\_future\_dataframe()}, and reads predictions back from
\texttt{predict()}. The same pattern repeats across the revenue, session-volume,
and member-activity forecasts; only the input series changes.
