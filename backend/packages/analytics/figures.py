...
def _prophet_test_preds(df, split, freq='D',
    ...
    from backend.packages.analytics.forecaster import (_build_holiday_df, _attach_weather,
...
def _xgboost_test_preds(df, split, use_weather=False, weekly_weather=False):
    from backend.packages.analytics.forecaster import _evaluate_xgboost
...
def _compute_all_predictions(tx: pd.DataFrame, split: float = 0.8) -> dict:
    ...
    from backend.packages.analytics.forecaster import _to_weekly
...
def _print_error_decomposition(all_preds: dict, forecasts_dir: str) -> None:
    ...
    from backend.packages.analytics.forecaster import holiday_week_starts
...
def _future_xgboost(weekly_hist: pd.DataFrame, horizon_weeks: int):
    from backend.packages.analytics.forecaster import _xgb_features
