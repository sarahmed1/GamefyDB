...
from backend.apps.api.services.data_cache import DataCache, EMPTY_COLUMNS
...
def test_build_pipeline_exception_logged_and_swallowed(caplog):
    cache = DataCache()
    with patch("backend.packages.etl.pipeline.run_pipeline", side_effect=RuntimeError("boom")):
...
def test_app_startup_builds_cache(monkeypatch):
    from backend.apps.api.services.data_cache import cache as singleton
