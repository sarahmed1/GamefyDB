...
# Clean up any incorrect backend modules from sys.modules to prevent collisions
for name, mod in list(sys.modules.items()):
    if name == 'backend' or name.startswith('backend.'):
        if hasattr(mod, '__file__') and mod.__file__ and ('backend\\backend' in mod.__file__ or 'backend/backend' in mod.__file__):
            sys.modules.pop(name, None)

# Ensure project root is in sys.path so backend imports work under Streamlit
...
from backend.apps.chatbot.chart_generator import make_chart
from backend.apps.chatbot.voice_component import voice_input
...
@st.cache_resource
def get_data():
    from backend.apps.chatbot.data_loader import load_data, recent_anomaly_count
    ctx = load_data()
...
with col_chat:
    ...
    if query:
        ...
        from backend.apps.chatbot.agent import ask
        try:
